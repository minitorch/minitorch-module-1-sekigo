...
class Network(minitorch.Module):
    def __init__(self, hidden_layers):
        super().__init__()
        # hidden_layers — количество нейронов в скрытом слое
...
class Linear(minitorch.Module):
    ...
    def forward(self, inputs):
        # inputs — кортеж или список Scalars длины in_size
        out = []
        for j in range(len(self.bias)):
            # линейная комбинация входов + bias
            z = self.bias[j]
            for i in range(len(inputs)):
                z += inputs[i] * self.weights[i][j]
            out.append(z)
        return out
